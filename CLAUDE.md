...
- **M2 · `plugin`** (ядро синка) — готов. Пакет [plugin/](plugin/): ядро на портах `WebDavClient`/`VaultWriter`/`StateStore` ([engine.ts](plugin/src/engine.ts) — `SyncEngine.run`: скачать+расшифровать манифест → diff → per-file GET+decrypt+write, guard `isSyncing`, пофайловый try/catch, неверный passphrase → throw до записи, `onProgress`), [diff.ts](plugin/src/diff.ts)/[paths.ts](plugin/src/paths.ts), Obsidian-адаптеры [webdav.ts](plugin/src/webdav.ts) (`requestUrl` инжектится, `btoa` для Basic), [vault-writer.ts](plugin/src/vault-writer.ts) (рекурсивный mkdir), [state-store.ts](plugin/src/state-store.ts). `manifest.json` c `isDesktopOnly:false`. esbuild → `main.js` (obsidian external, crypto-core инлайнится). Проверен сквозной e2e M1→M2 против реального WebDAV. План: [docs/superpowers/plans/2026-07-02-m2-plugin.md](docs/superpowers/plans/2026-07-02-m2-plugin.md).
- **M3 · `plugin`** (UX) — готов. Чистая логика TDD: [interval.ts](plugin/src/interval.ts) (пресеты 0/5/15/30/60), [status.ts](plugin/src/status.ts) (`formatRelativeTime`/`statusText`), [scheduler.ts](plugin/src/scheduler.ts) (на инжектируемых `Timers`), [validate.ts](plugin/src/validate.ts) (`validateSettings`/`testConnection`), прогресс движка. Рантайм: [status-ui.ts](plugin/src/status-ui.ts) (статус-бар), полный [settings.ts](plugin/src/settings.ts) (интервал-дропдаун, sync-on-open, Test connection, валидация), [main.ts](plugin/src/main.ts) (Scheduler, sync-on-open через `onLayoutReady`, ribbon-иконка, команды `Sync now` + `Reset local sync state`). Новые настройки: `syncInterval`, `syncOnOpen`. План: [docs/superpowers/plans/2026-07-02-m3-scheduler-settings-statusui.md](docs/superpowers/plans/2026-07-02-m3-scheduler-settings-statusui.md).
- **M4 · edge-cases + релиз** — готов (кроме ручного iOS-теста). Условный GET манифеста по ETag (`getConditional` + `If-None-Match` → `304` короткий путь «ничего не менялось», ETag хранится в state); offline-статус (`navigator.onLine` guard + классификация в catch); `SyncStats.notModified` → Notice «Already up to date». Релиз-обвязка: [.github/workflows/release.yml](.github/workflows/release.yml) (тег → сборка → GitHub-релиз с `main.js`/`manifest.json`/`styles.css`/`versions.json`), [README.md](README.md) (BRAT + деплой encryptor). Атомарная запись **отложена** осознанно (self-healing по hash достаточно для v1). 93 теста всего + typecheck + оба билда зелёные. План: [docs/superpowers/plans/2026-07-02-m4-edgecases-release.md](docs/superpowers/plans/2026-07-02-m4-edgecases-release.md).

- **M5 · Yandex.Disk REST backend** — готов. Второй бэкенд рядом с WebDAV (у Яндекса на free-тарифе WebDAV-запись = 402, а REST работает). Порты `WebDav`/`WebDavClient` переиспользованы, крипта/манифест/движок не тронуты. encryptor: [yandex.ts](encryptor/src/yandex.ts) (`YandexWebDav`: ensure-folder → upload href → PUT; DELETE resources), [backend.ts](encryptor/src/backend.ts) (фабрика по `config.backend`), конфиг `backend`/`yandexToken`/`remoteBase` (+ env `BACKEND`/`YANDEX_TOKEN`/`REMOTE_BASE`), ветвистая валидация. plugin: [yandex.ts](plugin/src/yandex.ts) (`YandexClient`: download href → GET; `getConditional` через `md5` метаданных вместо HTTP-ETag), `makeClient()` в [main.ts](plugin/src/main.ts), в настройках дропдаун Backend + поле OAuth-токена, `validateSettings`/testConnection по бэкенду. Auth: `Authorization: OAuth <token>`, пути `disk:/<remoteBase>/<name>`. Device-flow `--login` для headless-обновления токена **отложен** (пока токен из браузерного implicit-flow вставляется руками). 108 тестов всего + typecheck + оба билда зелёные. План: [docs/superpowers/plans/2026-07-02-m5-yandex-backend.md](docs/superpowers/plans/2026-07-02-m5-yandex-backend.md).

**Остаётся пользователю:** создать git-тег → GitHub Actions соберёт релиз; добавить репозиторий в BRAT; ручной e2e на iOS. Токен Яндекса живёт ~год; для VPS-cron позже можно добавить device-flow `--login`.
...
**UX (M3):** Scheduler на инжектируемых `Timers` (в проде `window.setInterval`), sync-on-open через `workspace.onLayoutReady`, статус-бар через `StatusUI`, ribbon-иконка, команды `Sync now` / `Reset local sync state`. Настройки: интервал (0=Off/5/15/30/60 мин), `syncOnOpen`, Test connection. Прогресс — `SyncEngine.onProgress` → `StatusUI.setSyncing(done,total)`. iOS: синк только на переднем плане (в UX-тексте настроек честно указано).

**M4:** условный GET манифеста по ETag (`WebDavClient.getConditional`, `If-None-Match`→304), offline-guard (`navigator.onLine`), `notModified`→«Already up to date». Релиз: `git tag <ver> && git push --tags` → [release.yml](.github/workflows/release.yml) собирает и прикладывает `main.js`/`manifest.json`/`styles.css`/`versions.json`. Атомарная запись отложена (см. [README](README.md) → Deferred).
