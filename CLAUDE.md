...
### `/plugin` M4 (ещё нет)
Edge-cases: offline-статус, опц. `If-None-Match` по ETag манифеста (короткий путь «ничего не менялось»), атомарная запись (temp+rename). Релиз BRAT: GitHub-релиз с `main.js`/`manifest.json`/`styles.css` + `versions.json`. Ручной e2e на iOS.
